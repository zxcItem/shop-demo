...
class FixShopConstraints extends Migrator
{
    /**
     * Change Method.
     *
    ...
    {
        // 为返佣记录表添加外键约束和业务字段
        $this->_fix_data_shop_user_rebate();

        // 为余额/积分表添加来源字段
        $this->_fix_data_payment_balance_integral();

        // 为用户关系表添加索引优化
        $this->_fix_data_shop_user_relation();

        // 为订单表添加检查约束
        $this->_fix_data_shop_order();
    }

    /**
     * 修复返佣记录表.
     */
    private function _fix_data_shop_user_rebate()
    {
        ...
        // 添加金额非负约束
        $this->executeModifyWithCheck('data_shop_user_rebate', 'amount', "DECIMAL(20,2) NOT NULL DEFAULT '0.00'", 'amount >= 0');
    }
     ...
     * 修复用户关系表索引.
     */
    private function _fix_data_shop_user_relation()
    {
        $indexes = $this->getTableIndexes('data_shop_user_relation');
        ...
        if (!$this->hasIndexDefinition($indexes, ['path'], false, ['path' => 20])) {
            foreach ($this->findIndexNamesByColumns($indexes, ['path']) as $name) {
                $this->execute("ALTER TABLE `data_shop_user_relation` DROP INDEX `{$name}`");
            }
            $this->execute('ALTER TABLE `data_shop_user_relation` ADD INDEX `idx_path_prefix` (`path`(20))');
            $indexes = $this->getTableIndexes('data_shop_user_relation');
        }
        ...
        foreach (['puid1', 'puid2', 'puid3'] as $column) {
            if (!$this->hasIndexDefinition($indexes, [$column])) {
                $this->execute("ALTER TABLE `data_shop_user_relation` ADD INDEX `idx_{$column}` (`{$column}`)");
                $indexes = $this->getTableIndexes('data_shop_user_relation');
            }
        }
    }
     ...
     * 修复订单表约束
     */
    private function _fix_data_shop_order()
    {
        // 添加金额字段的非负约束
        $amount_fields = [
        ...
        foreach ($amount_fields as $field) {
            $this->executeModifyWithCheck('data_shop_order', $field, "DECIMAL(20,2) NOT NULL DEFAULT '0.00'", "{$field} >= 0");
        }

        // 添加状态字段的枚举约束
        $this->executeModifyWithCheck('data_shop_order', 'status', 'TINYINT NOT NULL DEFAULT 1', 'status BETWEEN 0 AND 7');
        $this->executeModifyWithCheck('data_shop_order', 'refund_status', 'TINYINT NOT NULL DEFAULT 0', 'refund_status BETWEEN 0 AND 7');
        $this->executeModifyWithCheck('data_shop_order', 'payment_status', 'TINYINT NOT NULL DEFAULT 0', 'payment_status BETWEEN 0 AND 2');
        $this->executeModifyWithCheck('data_shop_order', 'delivery_type', 'TINYINT NOT NULL DEFAULT 0', 'delivery_type BETWEEN 0 AND 1');
    }
