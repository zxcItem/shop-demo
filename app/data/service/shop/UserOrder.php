...
use app\data\model\shop\DataShopOrderSender;
use app\data\model\shop\DataShopUserCreate;
...
abstract class UserOrder
{
    ...
    public static function stock(string $orderNo): bool
    {
        ...
        $codes = DataShopOrderItem::mk()->where($map)->column('gcode');
     ...
     * @throws Exception
     */
    public static function widthOrder($order, ?int &$unid = 0, ?string &$orderNo = ''): DataShopOrder
    {
        if (is_string($order)) {
            $order = DataShopOrder::mk()->where(['order_no' => $order])->findOrEmpty();
        }
        if ($order instanceof DataShopOrder) {
    ...
    public static function upgrade($order): ?array
    {
        ...
        $relation = DataShopUserRelation::mk()->where($where)->findOrEmpty();
        if ($relation->isEmpty()) {
            return null;
        }
        // 更新入会资格
    ...
    /**
     * 刷新用户入会礼包.
     * @param int|DataShopUserRelation $unid
     * @throws Exception
     * @throws DataNotFoundException
     * @throws DbException
     * @throws ModelNotFoundException
     */
    public static function entry($unid): DataShopUserRelation
    {
        [$relation, $unid] = DataShopUserRelation::withRelation($unid);
        // 订单升级等级
        $query = DataShopOrder::mk()->whereRaw('status>3 and refund_status<4');
        ...
        if (empty($enterAgent)) {
            ...
            if (DataShopUserCreate::mk()->where($map)->findOrEmpty()->isExists()) {
                $enterAgent = 1;
            }
        }
        $relation->save(['entry_agent' => $enterAgent, 'entry_member' => $enterMember]);
        // 触发代理注册
        $event = $enterAgent ? 'PluginShopAgentCreate' : 'PluginShopAgentCancel';
    ...
    public static function discount(int $disId, int $levelCode, float $disRate = 100.00): array
    {
        if ($disId > 0) {
            ...
            $discount = DataShopConfigDiscount::mk()->where($where)->findOrEmpty();
     ...
     * @throws Exception
     */
    public static function perfect(DataShopOrder $order, DataPaymentAddress $address): bool
    {
        ...
        [$amount, $tCount, $tCode, $remark] = ExpressService::amount(
            DataShopOrderItem::mk()->where($map1)->column('delivery_code'),
            $address->getAttr('region_prov'),
            $address->getAttr('region_city'),
            (int)DataShopOrderItem::mk()->where($map2)->sum('delivery_count')
        );
        // 创建订单发货信息
        $data = [
        ...
        // 记录原地址信息
        $data['extra'] = $data;
        DataShopOrderSender::mk()->where(['order_no' => $orderNo])->findOrEmpty()->save($data);
        // 组装更新订单数据, 重新计算订单金额
        ...
        if ($order->save($update)) {
            // 触发订单确认事件
            Library::$sapp->event->trigger('PluginShopOrderPerfect', $order);
            // 返回处理成功数据
            return true;
        }
        return false;
    ...
    /**
     * 更新订单支付状态
     * @param DataShopOrder|string $order 订单模型
