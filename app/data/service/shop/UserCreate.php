...
abstract class UserCreate
{
    /**
     * 创建账号及返佣.
     * @param int|DataShopUserCreate|string $user
     * @throws Exception
     */
            ...
            try {
                ...
                Library::$sapp->db->transaction(function () use ($user, $data) {
                    ...
                    if (!empty($data['agent_phone'])) {
                        ...
                        $relation = DataShopUserRelation::mk()->where(['unid' => $parent->getAttr('id')])->findOrEmpty();
                    ...
                    ($rebate = DataShopUserRebate::mk()->where($map)->findOrEmpty())->save([
                    ...
                    ($transfer = DataShopUserTransfer::mk()->where($map)->findOrEmpty())->save([
                        'unid' => $account->getUnid(),
                        'type' => 'platform',
                        'date' => date('Y-m-d'),
                        'code' => $map['code'],
    ...
    /**
     * 标准化模型.
     * @param int|DataShopUserCreate|string $model
     * @throws Exception
     */
    public static function withModel($model): DataShopUserCreate
    {
        if (is_numeric($model)) {
            return DataShopUserCreate::mk()->findOrEmpty($model);
        }
        if ($model instanceof DataShopUserCreate) {
            return $model;
        }
