...
abstract class UserCoupon
{
     ...
     */
    public static function create($unid, int $coid): DataShopUserCoupon
    {
        [$rela, $unid] = DataShopUserRelation::withRelation($unid);
        // 检查卡券
        $where = ['id' => $coid, 'status' => 1, 'deleted' => 0];
        $coupon = DataShopConfigCoupon::mk()->where($where)->findOrEmpty();
        if ($coupon->isEmpty()) {
        ...
        if (($limitTimes = $coupon->getAttr('limit_times')) > 0) {
            ...
            if (DataShopUserCoupon::mk()->where($map)->count() > $limitTimes) {
        ...
        } while (($model = DataShopUserCoupon::mk()->where(['code' => $code])->findOrEmpty())->isExists());
        // 保存及返回模型
    ...
    /**
     * 重置卡券统计
     * @param int|DataShopConfigCoupon $coid
     * @throws Exception
     */
    public static function recount($coid): bool
    {
        ...
        $total = DataShopUserCoupon::mk()->field($field)->where($where)->findOrEmpty()->toArray();
     ...
     * 恢复优惠券.
     */
    public static function resume(string $code): DataShopUserCoupon
    {
        $coupon = DataShopUserCoupon::mk()->where(['code' => $code, 'status' => 2])->findOrEmpty();
        if ($coupon->isExists()) {
     ...
     * @throws Exception
     */
    public static function confirm(string $code): DataShopUserCoupon
    {
        $map = ['code' => $code, 'status' => 1];
        if (($coupon = DataShopUserCoupon::mk()->where($map)->findOrEmpty())->isExists()) {
    ...
    /**
     * 获取优惠券模型.
     * @param int|DataShopConfigCoupon $model
     * @throws Exception
     */
    public static function withModel($model): DataShopConfigCoupon
    {
        if (is_numeric($model)) {
            $model = DataShopConfigCoupon::mk()->where(['id' => $model])->findOrEmpty();
        }
        if ($model instanceof DataShopConfigCoupon) {
            if ($model->isExists()) {
                return $model;
            }
