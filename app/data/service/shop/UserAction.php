...
use app\data\model\shop\DataShopUserActionHistory;
...
abstract class UserAction
{
    ...
    public static function set(int $unid, string $gcode, string $type): array
    {
        ...
        if ($type === 'collect') {
            $model = DataShopUserActionCollect::mk()->where($data)->findOrEmpty();
        } else {
            $model = DataShopUserActionHistory::mk()->where($data)->findOrEmpty();
        }
        $data['sort'] = time();
    ...
    public static function del(int $unid, string $gcode, string $type): array
    {
        ...
        if ($type === 'collect') {
            DataShopUserActionCollect::mk()->where($data)->delete();
        } else {
            DataShopUserActionHistory::mk()->where($data)->delete();
        }
        self::recount($unid);
        return $data;
    }
    ...
    public static function recount(int $unid, ?array &$data = null): array
    {
        ...
        // 更新收藏及足迹数量和购物车
        $map = ['unid' => $unid];
        $data['mycarts_total'] = DataShopOrderCart::mk()->where($map)->sum('number');
        $data['collect_total'] = DataShopUserActionCollect::mk()->where($map)->count();
        $data['history_total'] = DataShopUserActionHistory::mk()->where($map)->count();
     ...
     * @throws Exception
     */
    public static function comment(DataShopOrderItem $item, $rate, string $content, string $images): bool
    {
        // 图片上传转存
        if (!empty($images)) {
        ...
        return DataShopUserActionComment::mk()->where(['code' => $code])->findOrEmpty()->save([
            'code' => $code,
