...
abstract class UserAgent
{
    /**
     * 同步计算代理等级.
     * @param int|DataShopUserRelation $unid 指定用户UID
    ...
    public static function upgrade($unid, bool $parent = true, ?string $orderNo = null): DataShopUserRelation
    {
        ...
        // 筛选会员等级
        $mLevels = DataShopConfigLevel::mk()->where(['status' => 1, 'upgrade_team' => 1])->column('number');
        // 统计团队数据
        $model = DataShopUserRelation::mk()->where(['level_code' => $mLevels]);
        ...
        $amountTotal = DataShopOrder::mk()->whereRaw("unid in {$relaSql}")->where("unid={$unid} and status>=4")->sum('amount_total');
        // 直接及间接团队金额(不含自己)
        $relaSql = (clone $model)->where(['puid1' => $unid])->field('unid')->buildSql();
        $amountDirect = DataShopOrder::mk()->whereRaw("puid1 in {$relaSql}")->where("unid={$unid} and status>=4")->sum('amount_total');
        $amountIndirect = DataShopOrder::mk()->whereRaw("puid2 in {$relaSql}")->where("unid={$unid} and status>=4")->sum('amount_total');
        // 通过订单升级等级
        $map = ['unid' => $unid, 'payment_status' => 1];
        $tmpCode = DataShopOrder::mk()->where($map)->where('status', '>', 4)->max('level_agent');
        // 动态计算会员等级
        [$levelName, $levelCode, $levelCurr] = ['会员用户', 0, intval($rela->getAttr('agent_level_code'))];
        foreach (DataShopConfigAgent::mk()->where(['status' => 1])->order('number desc')->select()->toArray() as $item) {
        ...
        $levelCurr < $levelCode && Library::$sapp->event->trigger('PluginShopUpgradeAgent', [
