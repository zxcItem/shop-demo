...
abstract class GoodsService
{
    ...
    {
        // 入库统计
        $query = DataShopGoodsStock::mk()->field('ghash,ifnull(sum(gstock),0) stock_total');
        $stockList = $query->where(['gcode' => $code])->group('gcode,ghash')->select()->toArray();
        // 销量统计
        $query = DataShopOrder::mk()->alias('a')->field('b.ghash,ifnull(sum(b.stock_sales),0) stock_sales');
        $query->join([DataShopOrderItem::mk()->getTable() => 'b'], 'a.order_no=b.order_no', 'left');
    ...
    public static function parse(int $unid, string $rules, string $carts): array
    {
        ...
        if (!empty($carts)) {
            ...
            DataShopOrderCart::mk()->field($field)->where($where)->with([
            ...
            foreach (DataShopGoodsItem::mk()->where($map1)->select()->toArray() as $item) {
            ...
            foreach (DataShopGoods::mk()->where($map2)->withoutField(['specs', 'content'])->select()->toArray() as $goods) {
                foreach ($lines as &$line) {
