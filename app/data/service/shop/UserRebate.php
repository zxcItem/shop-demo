...
namespace app\data\service\shop;
...
use app\data\model\shop\DataShopConfigRebate;
...
abstract class UserRebate
{
    ...
    /**
     * 执行订单返佣处理.
     * @param DataShopOrder|string $order
     * @throws Exception
    ...
    public static function create($order)
    {
        ...
        self::$rela0 = DataShopUserRelation::mk()->where(['unid' => self::$unid])->findOrEmpty()->toArray();
        ...
        if (self::$order['puid1'] > 0) {
            ...
            self::$rela1 = DataShopUserRelation::mk()->where($map)->findOrEmpty()->toArray();
            if (empty(self::$rela1)) {
        ...
        if (self::$order['puid2'] > 0) {
            ...
            self::$rela2 = DataShopUserRelation::mk()->where($map)->findOrEmpty()->toArray();
            if (empty(self::$rela2)) {
        ...
        if (self::$order['puid3'] > 0) {
            ...
            self::$rela3 = DataShopUserRelation::mk()->where($map)->findOrEmpty()->toArray();
            if (empty(self::$rela3)) {
        ...
        DataShopConfigRebate::mk()->where($where)->order('sort desc,id desc')->select()->map(function (DataShopConfigRebate $item) {
    ...
    public static function confirm($order): bool
    {
        ...
        /** @var DataShopUserRebate $item */
        $map = [['status', '=', 0], ['deleted', '=', 0], ['order_no', 'like', "{$order->getAttr('order_no')}%"]];
        foreach (DataShopUserRebate::mk()->where($map)->cursor() as $item) {
    ...
    public static function recount(int $unid, ?array &$data = null, $where = []): array
    {
        ...
        if ($unid > 0) {
            $total = DataShopUserRebate::mk()->where($where)->whereRaw("unid='{$unid}' and deleted=0")->sum('amount');
            $count = DataShopUserTransfer::mk()->where($where)->whereRaw("unid='{$unid}' and status>0")->sum('amount');
            $locks = DataShopUserRebate::mk()->where($where)->whereRaw("unid='{$unid}' and status=0 and deleted=0")->sum('amount');
            ...
            $total = DataShopUserRebate::mk()->where($where)->whereRaw('deleted=0')->sum('amount');
            $count = DataShopUserTransfer::mk()->where($where)->whereRaw('status>0')->sum('amount');
            $locks = DataShopUserRebate::mk()->where($where)->whereRaw('status=0 and deleted=0')->sum('amount');
            $usable = bcsub(bcsub(strval($total), strval($count), 2), strval($locks), 2);
            [$data['rebate_total'], $data['rebate_used'], $data['rebate_lock'], $data['rebate_usable']] = [$total, $count, $locks, $usable];
        }
    ...
    public static function levels(): array
    {
        // 解析商品折扣规则
        $discs = [];
        foreach (DataShopConfigDiscount::items() as $v) {
            foreach ($v['items'] as $vv) {
                $discs[$vv['level']][] = strval($vv['discount']);
            }
        }
        // 合并等级折扣及奖励
        $levels = DataShopConfigLevel::items(null);
    ...
    protected static function _order(int $layer, array $relation): bool
    {
        ...
        if (DataShopUserRebate::mk()->where($map)->findOrEmpty()->isExists()) {
            return false;
        }
        // 根据配置计算返利数据
    ...
    {
        // 是否首次购买
        $orders = DataShopUserRebate::mk()->where(['order_unid' => self::$unid, 'status' => 1])->limit(2)->column('order_no');
    ...
    {
        $rebate = DataShopUserRebate::mk()->where($map)->findOrEmpty();
        if ($rebate->isExists()) {
            return false;
        }
        if (self::$status) {
