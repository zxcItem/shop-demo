...
abstract class UserReward
{
     ...
     * @throws Exception
     */
    public static function create($order, ?string &$code = ''): DataShopOrder
    {
     ...
     * @throws Exception
     */
    public static function confirm($order, ?string &$code = ''): DataShopOrder
    {
        $order = UserOrder::widthOrder($order, $unid, $orderNo);
        if ($order->isEmpty() || $order->getAttr('status') < 4) {
            throw new Exception('订单状态异常');
     ...
     * @throws Exception
     */
    public static function cancel($order, ?string &$code = ''): DataShopOrder
    {
        $order = UserOrder::widthOrder($order, $unid, $orderNo);
