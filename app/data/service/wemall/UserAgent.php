...
abstract class UserAgent
{
    ...
    public static function upgrade($unid, bool $parent = true, ?string $orderNo = null): DataWemallUserRelation
    {
        ...
        // 筛选会员等级
        $mLevels = DataWemallConfigLevel::mk()->where(['status' => 1, 'upgrade_team' => 1])->column('number');
        // 统计团队数据
        $model = DataWemallUserRelation::mk()->where(['level_code' => $mLevels]);
        ...
        $amountTotal = DataWemallOrder::mk()->whereRaw("unid in {$relaSql}")->where("unid={$unid} and status>=4")->sum('amount_total');
        // 直接及间接团队金额(不含自己)
        $relaSql = (clone $model)->where(['puid1' => $unid])->field('unid')->buildSql();
        $amountDirect = DataWemallOrder::mk()->whereRaw("puid1 in {$relaSql}")->where("unid={$unid} and status>=4")->sum('amount_total');
        $amountIndirect = DataWemallOrder::mk()->whereRaw("puid2 in {$relaSql}")->where("unid={$unid} and status>=4")->sum('amount_total');
        // 通过订单升级等级
        $map = ['unid' => $unid, 'payment_status' => 1];
        $tmpCode = DataWemallOrder::mk()->where($map)->where('status', '>', 4)->max('level_agent');
        // 动态计算会员等级
        [$levelName, $levelCode, $levelCurr] = ['会员用户', 0, intval($rela->getAttr('agent_level_code'))];
        foreach (DataWemallConfigAgent::mk()->where(['status' => 1])->order('number desc')->select()->toArray() as $item) {
