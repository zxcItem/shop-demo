...
use app\data\service\payment\payment\AliPayment;
use app\data\service\payment\payment\BalancePayment;
use app\data\service\payment\payment\CouponPayment;
use app\data\service\payment\payment\EmptyPayment;
use app\data\service\payment\payment\IntegralPayment;
use app\data\service\payment\payment\VoucherPayment;
use app\data\service\payment\payment\WechatPayment;
...
abstract class Payment
{
    ...
    public static function params(string $code, array $config = []): array
    {
        try {
            if (empty($config)) {
                ...
                $config = DataPaymentConfig::mk()->where($map)->findOrEmpty()->toArray();
            }
            if (empty($config)) {
    ...
    public static function typesByAccess(string $account, bool $getfull = false): array
    {
        ...
        if ($getfull) {
            $items = [];
            $query = DataPaymentConfig::mk()->field('type,code,name,cover,content');
    ...
    public static function items(): array
    {
        ...
        return DataPaymentConfig::mk()->where($map)->order('sort desc,id desc')->column('type,code,name', 'code');
    }
    ...
    {
        $items = DataPaymentRecord::mq()->where(function (Query $query) {
    ...
    public static function paidAmount(string $orderNo, bool $realtime = false): string
    {
        ...
        return bcadd('0.00', strval(DataPaymentRecord::mk()->where($map)->sum($raw)), 2);
    }
    ...
    public static function totalPaymentAmount(string $orderNo): array
    {
        ...
        try {
            DataPaymentRecord::mk()->where(['order_no' => $orderNo, 'payment_status' => 1])->field([
                'channel_type',
            ...
            ])->group('channel_type')->select()->map(static function (DataPaymentRecord $item) use (&$total) {
    ...
    public static function totalRefundAmount(string $pCode): array
    {
        $total = ['amount' => '0.00', 'payment' => '0.00', 'balance' => '0.00', 'integral' => '0.00'];
        try {
            DataPaymentRefund::mk()->where(['record_code' => $pCode, 'refund_status' => [0, 1]])->field([
                'refund_account', 'sum(refund_amount) amount', 'sum(used_payment)' => 'payment', 'sum(used_balance)' => 'balance', 'sum(used_integral)' => 'integral',
            ])->group('refund_account')->select()->map(static function (DataPaymentRefund $item) use (&$total) {
                $total['amount'] = bcadd($total['amount'], strval($item->getAttr('amount')), 2);
    ...
    public static function withPaymentCode(): string
    {
        ...
        } while (DataPaymentRecord::mk()->master()->where($data)->findOrEmpty()->isExists());
        return $data['code'];
    }
    ...
    public static function withRefundCode(): string
    {
        ...
        } while (DataPaymentRefund::mk()->master()->where($data)->findOrEmpty()->isExists());
        return $data['code'];
    }
