...
namespace app\data\service\payment\contract;
...
trait PaymentUsageTrait
{
     ...
     * @throws Exception
     */
    public static function syncRefund(string $pCode, ?string &$rCode = '', ?string $amount = null, string $reason = ''): DataPaymentRecord
    {
        // 检查退款单号
        if ($rCode && DataPaymentRefund::mk()->where(['code' => $pCode])->findOrEmpty()->isExists()) {
        ...
        if (bccomp(bcadd($currentRefundAmount, $refundAmountFloat, 2), strval($record->getAttr('payment_amount')), 2) <= 0) {
            DataPaymentRefund::mk()->save(array_merge([
    ...
    protected function checkLeaveAmount($orderNo, $payAmount, $orderAmount): string
    {
        ...
        $model = DataPaymentRecord::mk()->where($map)->findOrEmpty();
        if ($model->isExists()) {
    ...
    protected function createAction(string $orderNo, string $orderTitle, string $orderAmount, string $payCode, string $payAmount, string $payImages = '', string $usedBalance = '0.00', string $usedIntegral = '0.00'): array
    {
        ...
        $map['code'] = $payCode;
        if (($model = DataPaymentRecord::mk()->where($map)->findOrEmpty())->isExists()) {
    ...
    protected function updateAction(string $pCode, string $pTrade, string $pAmount, ?string $pRemark = '在线支付', ?string $pCoupon = null, ?array $pNotify = null)
    {
        ...
        if (($model = DataPaymentRecord::mk()->where($map)->findOrEmpty())->isEmpty()) {
            return false;
        }
        $data = [
            'code' => $pCode,
    ...
    /**
     * 获取并同步退款金额的支付单.
     * @param DataPaymentRecord|string $record
     * @throws Exception
     */
    protected static function withPaymentByRefundTotal($record): DataPaymentRecord
    {
        if (is_string($record)) {
            $record = DataPaymentRecord::mk()->where(['code' => $record])->findOrEmpty();
        }
        if (!$record instanceof DataPaymentRecord || $record->isEmpty()) {
