 ...
 * @property DataWemallConfigDiscount $discount
 * @property DataWemallGoodsItem[] $items
 * @property DataWemallUserActionComment[] $comments
 * @class DataWemallGoods
 */
class DataWemallGoods extends Abs
{
    /**
     * 日志名称.
     * @var string
    ...
    {
        return $this->hasMany(DataWemallGoodsItem::class, 'gcode', 'code')
    ...
    {
        return $this->hasMany(DataWemallUserActionComment::class, 'gcode', 'code')->with('bindUser');
    }
    ...
    {
        return $this->hasOne(DataWemallConfigDiscount::class, 'id', 'discount_id')->where(['status' => 1, 'deleted' => 0])->field('id,name,items');
    }
    ...
    public function getMarksAttr($value): array
    {
        ...
        $items = sysvar($ckey) ?: sysvar($ckey, DataWemallGoodsMark::items());
    ...
    public function getCatesAttr($value): array
    {
        ...
        $cates = sysvar($ckey) ?: sysvar($ckey, DataWemallGoodsCate::items(true));
