...
use app\data\model\shop\DataShopConfigAgent;
use app\data\model\shop\DataShopConfigDiscount;
use app\data\model\shop\DataShopConfigLevel;
use app\data\model\shop\DataShopExpressTemplate;
use app\data\model\shop\DataShopGoods;
use app\data\model\shop\DataShopGoodsCate;
use app\data\model\shop\DataShopGoodsItem;
use app\data\model\shop\DataShopGoodsMark;
use app\data\model\shop\DataShopGoodsStock;
use app\data\service\shop\ConfigService;
...
class Goods extends Base
{
    ...
    public function index()
    {
        ...
        DataShopGoods::mQuery($this->get)->layTable(function () {
            $this->title = '商品数据管理';
            $this->cates = DataShopGoodsCate::items();
            $this->marks = DataShopGoodsMark::items();
            $this->agents = DataShopConfigAgent::items();
            $this->upgrades = DataShopConfigLevel::items('普通商品');
            $this->deliverys = DataShopExpressTemplate::items(true);
    ...
    public function stock()
    {
        ...
        if ($this->request->isGet()) {
            $this->vo = DataShopGoods::mk()->where($input)->with('items')->findOrEmpty()->toArray();
            ...
            try {
                ...
                if (isset($post['gcode']) && is_array($post['gcode'])) {
                    ...
                    empty($data) || DataShopGoodsStock::mk()->saveAll($data);
                }
    ...
    {
        DataShopGoods::mSave($this->_vali([
    ...
    protected function _form_filter(array &$data)
    {
        ...
        if ($this->request->isGet()) {
            $this->marks = DataShopGoodsMark::items();
            $this->cates = DataShopGoodsCate::items(true);
            $this->agents = DataShopConfigAgent::items();
            $this->upgrades = DataShopConfigLevel::items('普通商品');
            $this->discounts = DataShopConfigDiscount::items(true);
            $this->deliverys = DataShopExpressTemplate::items(true);
            $this->enableBalance = ConfigService::get('enable_balance');
            $this->enableIntegral = ConfigService::get('enable_integral');
            ...
            $data['items'] = DataShopGoodsItem::itemsJson($data['code']);
            ...
            try {
                ...
                $this->app->db->transaction(static function () use ($data, $items) {
                    ...
                    $model = DataShopGoods::mk()->where(['code' => $data['code']])->findOrEmpty();
                    $model->{$model->isExists() ? 'onAdminUpdate' : 'onAdminInsert'}($data['code']);
                    $model->save($data);
                    // 更新或写入商品规格
                    foreach ($items as $item) {
                        DataShopGoodsItem::mUpdate([
                            'gsku' => $item['gsku'],
                            'ghash' => $item['hash'],
                            'gcode' => $data['code'],
                            'gspec' => $item['spec'],
