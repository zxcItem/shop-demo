...
class Report extends Controller
{
    ...
    {
        $this->usersTotal = DataShopUserRelation::mk()->cache(true, 60)->count();
        $this->goodsTotal = DataShopGoods::mk()->cache(true, 60)->where(['deleted' => 0])->count();
        $this->orderTotal = DataShopOrder::mk()->cache(true, 60)->whereRaw('status >= 4')->count();
        $this->amountTotal = DataShopOrder::mk()->cache(true, 60)->whereRaw('status >= 4')->sum('amount_total');

        // 近十天的用户及交易趋势
        if (empty($this->days = $this->app->cache->get('Data.wemall.portals', []))) {
            $field = ['count(1)' => 'count', 'substr(create_time,1,10)' => 'mday'];
            // 统计用户数据
            $model = DataShopUserRelation::mk()->field($field);
            $users = $model->whereTime('create_time', '-10 days')->group('mday')->select()->column(null, 'mday');
            // 统计订单数据
            $model = DataShopOrder::mk()->field($field + ['sum(amount_total)' => 'amount']);
            $orders = $model->whereRaw('status>=4')->whereTime('create_time', '-10 days')->group('mday')->select()->column(null, 'mday');
            // 统计返佣数据
            $model = DataShopUserRebate::mk()->field($field + ['sum(amount)' => 'amount']);
        ...
        // 会员级别分布统计
        $levels = DataShopConfigLevel::mk()->where(['status' => 1])->order('number asc')->column('number code,name,0 count', 'number');
        foreach (DataShopUserRelation::mk()->field('count(1) count,level_code level')->group('level_code')->cursor() as $vo) {
