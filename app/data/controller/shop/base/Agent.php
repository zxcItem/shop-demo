...
class Agent extends Controller
{
    ...
    {
        DataShopConfigAgent::mQuery()->layTable(function () {
            $this->title = '代理等级管理';
    ...
    public function add()
    {
        $this->max = DataShopConfigAgent::maxNumber() + 1;
        DataShopConfigAgent::mForm('form');
    }
    ...
    {
        $this->max = DataShopConfigAgent::maxNumber();
        DataShopConfigAgent::mForm('form');
    }
    ...
    public function _form_result(bool $state)
    {
        if ($state) {
            ...
            foreach (DataShopConfigAgent::mk()->order($order)->select() as $number => $upgrade) {
    ...
    protected function _form_filter(array &$vo)
    {
        if ($this->request->isGet()) {
            ...
            $vo['number'] = $vo['number'] ?? DataShopConfigAgent::maxNumber();
        } else {
            $count = 0;
