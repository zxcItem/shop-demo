...
class Admin extends Controller
{
    ...
    public function index()
    {
        ...
        DataShopUserRelation::mQuery()->layTable(function () {
            $this->title = '会员用户管理';
            $this->upgrades = DataShopConfigLevel::items();
            ...
            // 用户内容查询
            $user = DataAccountUser::mQuery()->dateBetween('create_time');
    ...
    {
        DataShopUserRelation::mQuery()->with('user')->mForm('form', 'unid');
    }
    ...
    public function parent()
    {
        if ($this->request->isGet()) {
            ...
            try {
                ...
                $parent = DataShopUserRelation::mQuery()->where(['unid' => $data['puid']])->findOrEmpty();
                if ($parent->isEmpty()) {
                    $this->error('上级用户不存在！');
                }
                $relation = DataShopUserRelation::withInit(intval($data['unid']));
