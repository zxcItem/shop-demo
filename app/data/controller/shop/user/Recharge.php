...
use app\data\model\shop\DataShopUserRecharge;
...
class Recharge extends Controller
{
    ...
    {
        DataShopUserRecharge::mQuery()->layTable(function () {
            $this->title = '会员充值管理';
            $this->total = DataShopUserRecharge::mk()->where(['deleted' => 0])->sum('amount');
    ...
    public function add()
    {
        DataShopUserRecharge::mForm('form');
    }
    ...
    public function remove()
    {
        try {
            ...
            $recharge = DataShopUserRecharge::mk()->where($data)->findOrEmpty();
            if ($recharge->isEmpty()) {
                $this->error('待删除记录不存在！');
            }
        ...
        if ($this->request->isGet()) {
            ...
            try {
                ...
                $this->app->db->transaction(static function () use ($data) {
                    ...
                    // 创建充值记录
                    DataShopUserRecharge::mk()->where(['code' => $data['code']])->findOrEmpty()->save($data);
                    // 创建余额变更
