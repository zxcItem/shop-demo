...
use app\data\service\shop\UserRefund;
...
class Order extends Controller
{
    ...
    public function index()
    {
        ...
        DataShopOrder::mQuery()->layTable(function (QueryHelper $query) {
            $this->title = '订单数据管理';
            ...
            // 发货信息搜索
            $db = DataShopOrderSender::mQuery()->dateBetween('express_time')
            ...
            // 代理搜索查询
            $db = DataAccountUser::mQuery()->like('phone|nickname#from_keys')->db();
            if ($db->getOptions('where')) {
    ...
    public function audit()
    {
        if ($this->request->isGet()) {
            ...
            $order = DataShopOrder::mk()->where(['order_no' => $data['order_no']])->findOrEmpty();
            ...
            if (DataShopOrder::mk()->strict(false)->where($map)->update($data) !== false) {
                if (in_array($data['status'], [4, 5, 6])) {
                    $this->app->event->trigger('DataPaymentSuccess', $data);
    ...
    public function cancel()
    {
        ...
        $order = DataShopOrder::mk()->where($data)->findOrEmpty();
        if ($order->isEmpty()) {
            $this->error('订单查询异常！');
        }
        try {
