...
use app\data\service\shop\UserCoupon;
...
class Refund extends Controller
{
    ...
    public function index()
    {
        ...
        DataShopOrderRefund::mQuery()->layTable(function (QueryHelper $query) {
            $this->title = '售后订单管理';
    ...
    protected function _form_filter(array &$data)
    {
        if ($this->request->isGet()) {
            ...
            try {
                $refund = DataShopOrderRefund::mk()->findOrEmpty($data['id'] ?? 0);
                if ($refund->isEmpty()) {
                    $this->error('无效的售后单！');
                }
                    ...
                    foreach ($data['ptypes'] as $pcode => $type) {
                        if (bccomp(strval($data['refunds'][$pcode]), '0.00', 2) > 0) {
                            ...
                            if ($type === Payment::INTEGRAL) {
                                ...
                                $coupon = DataPaymentRecord::mk()->where($map)->findOrEmpty()->toArray();
