...
use app\data\service\wemall\UserTransfer;
...
class Transfer extends Auth
{
    ...
    public function add()
    {
        ...
        $count = DataWemallUserTransfer::mk()->where($map)->count();
        ...
        // 写入代理提现数据
        if (DataWemallUserTransfer::mk()->save($data)) {
    ...
    public function get()
    {
        $query = DataWemallUserTransfer::mQuery()->where(['unid' => $this->unid]);
        ...
        $this->success('获取提现成功', array_merge($result, [
            'total' => [
                ...
                '锁定' => $locks,
                '可提' => $usable,
                '上月' => DataWemallUserTransfer::mk()->where($map)->whereLike('date', date('Y-m-%', strtotime('-1 month')))->sum('amount'),
                '本月' => DataWemallUserTransfer::mk()->where($map)->whereLike('date', date('Y-m-%'))->sum('amount'),
                '全年' => DataWemallUserTransfer::mk()->where($map)->whereLike('date', date('Y-%'))->sum('amount'),
            ],
        ]));
    }
    ...
    public function cancel()
    {
        ...
        DataWemallUserTransfer::mk()->where($data)->whereIn('status', [1, 2, 3])->update([
    ...
    {
        $data = $this->_vali(['unid.value' => $this->unid, 'code.require' => '单号不能为空！']);
        DataWemallUserTransfer::mk()->where($data)->whereIn('status', [4])->update([
