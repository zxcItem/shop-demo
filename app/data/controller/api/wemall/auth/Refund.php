...
class Refund extends Auth
{
    ...
    public function get()
    {
        DataWemallOrderRefund::mQuery(null, function (QueryHelper $query) {
    ...
    public function add()
    {
        ...
        if (($refund = DataWemallOrderRefund::mk())->save($data)) {
    ...
    public function express()
    {
        ...
        $data['express_name'] = DataWemallExpressCompany::mk()->where($map)->value('name');
        if (empty($data['express_name'])) {
            $this->error('无效快递公司！');
        }
        // 更新售后内容
        self::saveRefund(function (DataWemallOrderRefund $refund) use ($data) {
