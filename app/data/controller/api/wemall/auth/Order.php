...
class Order extends Auth
{
    ...
    public function get()
    {
        DataWemallOrder::mQuery(null, function (QueryHelper $query) {
    ...
    public function add()
    {
        try {
            ...
            } while (DataWemallOrder::mk()->master()->where($extra)->findOrEmpty()->isExists());
            ...
            foreach (GoodsService::parse($this->unid, trim($input['rules'], ':;'), $input['carts']) as $item) {
                ...
                if (isset($goods['limit_maxnum']) && $goods['limit_maxnum'] > 0) {
                    $join = [DataWemallOrderItem::mk()->getTable() => 'b'];
                    $where = [['a.unid', '=', $this->unid], ['a.status', '>', 1], ['b.gcode', '=', $goods['code']]];
                    $buyCount = DataWemallOrder::mk()->alias('a')->join($join, 'a.order_no=b.order_no')->where($where)->sum('b.stock_sales');
            ...
            // 写入商品数据
            $model = DataWemallOrder::mk();
            $this->app->db->transaction(function () use ($order, $items, &$model) {
                $model->save($order) && DataWemallOrderItem::mk()->saveAll($items);
                // 设置收货地址
            ...
            if (count($carts = str2arr($input['carts'])) > 0) {
                DataWemallOrderCart::mk()->whereIn('id', $carts)->delete();
                UserAction::recount($this->unid);
            }
            // 触发订单创建事件
            $this->app->event->trigger('DataWemallOrderCreate', $order);
            // 无需发货且无需支付，直接完成支付流程
    ...
    public function express()
    {
        ...
        $tCount = intval(DataWemallOrderItem::mk()->where($map)->sum('delivery_count'));

        // 根据地址计算运费
        $map = ['status' => 1, 'deleted' => 0, 'order_no' => $data['order_no']];
        $tCode = DataWemallOrderItem::mk()->where($map)->column('delivery_code');
    ...
    public function perfect()
    {
        ...
        $order = DataWemallOrder::mk()->where($where)->whereIn('status', [1, 2])->findOrEmpty();
        if ($order->isEmpty()) {
            $this->error('不能修改！');
        }
    ...
    public function cancel()
    {
        ...
        if (in_array($order->getAttr('status'), [1, 2, 3])) {
            ...
            if ($order->save($data) && UserOrder::stock($order->getAttr('order_no'))) {
                ...
                $this->app->event->trigger('DataWemallOrderCancel', $order);
                // 返回处理成功数据
                $this->success('取消成功！');
            } else {
                $this->error('取消失败！');
    ...
    public function remove()
    {
        ...
        if ($order->getAttr('status') == 0) {
            if ($order->save([
                ...
                // 触发订单删除事件
                $this->app->event->trigger('DataWemallOrderRemove', $order);
                // 返回处理成功数据
                $this->success('删除成功！');
            } else {
                $this->error('删除失败！');
    ...
    public function confirm()
    {
        ...
        if (in_array($order->getAttr('status'), [5, 6])) {
            ...
            $this->app->event->trigger('DataWemallOrderConfirm', $order);
            // 返回处理成功数据
            $this->success('确认成功！');
        } else {
            $this->error('确认失败！');
        ...
        if (in_array($order->getAttr('status'), [6, 7])) {
            try {
                ...
                $this->app->db->transaction(function () use ($order, $items) {
                    $order->save(['status' => 7]);
                    $order->items()->select()->map(function (DataWemallOrderItem $item) use ($items) {
    ...
    public function total()
    {
        ...
        $query = DataWemallOrder::mk()->where(['unid' => $this->unid, 'refund_status' => 0, 'deleted_status' => 0]);
     ...
     * 获取输入订单模型.
     */
    private function getOrderModel(): DataWemallOrder
    {
        $map = $this->_vali(['unid.value' => $this->unid, 'order_no.require' => '单号不能为空']);
        $order = DataWemallOrder::mk()->where($map)->findOrEmpty();
        if ($order->isEmpty()) {
            $this->error('读取订单失败！');
        }
        return $order;
