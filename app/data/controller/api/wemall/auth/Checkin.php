...
use app\data\model\wemall\DataWemallUserCheckin;
...
class Checkin extends Auth
{
    ...
    {
        try {
            $conf = sysdata(DataWemallUserCheckin::$ckcfg);
            if (empty($conf['status'])) {
                $this->error('活动未开始！');
            }
            $last = DataWemallUserCheckin::mk()->where(['unid' => $this->unid])->order('id desc')->findOrEmpty();
            ...
            ($checkin = DataWemallUserCheckin::mk())->save([
                'unid' => $this->unid,
                'date' => $this->today,
                'times' => $times,
                'timed' => $timed,
    ...
    public function get()
    {
        ...
        DataWemallUserCheckin::mQuery(null, function (QueryHelper $query) use ($date) {
    ...
    {
        $data = sysdata(DataWemallUserCheckin::$ckcfg);
