...
class Collect extends Auth
{
    ...
    public function set()
    {
        ...
        $goods = DataWemallGoods::mk()->where($map)->findOrEmpty();
        if ($goods->isExists()) {
    ...
    public function get()
    {
        DataWemallUserActionCollect::mQuery(null, function (QueryHelper $query) {
            // 关联商品信息
    ...
    {
        DataWemallUserActionCollect::mk()->where(['unid' => $this->unid])->delete();
