...
use app\data\model\wemall\DataWemallUserActionSearch;
...
class Goods extends Controller
{
    ...
    public function get()
    {
        $this->coupon = null;
        $this->cnames = null;
        DataWemallGoods::mQuery(null, function (QueryHelper $query) {
            // 根据优惠券展示商品
            if ($couponCode = input('coupon')) {
                $where = ['code' => $couponCode, 'deleted' => 0];
                $userCoupon = DataWemallUserCoupon::mk()->where($where)->findOrEmpty();
            ...
            if (!empty($vCates = input('cates'))) {
                $cates = array_filter(DataWemallGoodsCate::items(), function ($v) use ($vCates) {
                    return $v['id'] == $vCates;
                });
                $this->cnames = null;
                if (count($cates) > 0) {
            ...
            if (!empty($code = input('code'))) {
                ...
                DataWemallGoods::mk()->where(['code' => $code])->inc('num_read')->update([]);
            } else {
    ...
    {
        $this->success('获取分类成功', [
            'mark' => DataWemallGoodsMark::items(),
            'cate' => DataWemallGoodsCate::dtree(),
        ]);
    }
    ...
    {
        DataWemallUserActionComment::mQuery(null, function (QueryHelper $query) {
    ...
    {
        $query = DataWemallExpressCompany::mk()->where(['status' => 1, 'deleted' => 0]);
    ...
    {
        DataWemallUserActionSearch::mQuery(null, function (QueryHelper $query) {
