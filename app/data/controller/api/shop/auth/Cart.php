...
class Cart extends Auth
{
    ...
    public function get()
    {
        DataShopOrderCart::mQuery(null, function (QueryHelper $query) {
    ...
    public function set()
    {
        ...
        if ($data['number'] < 1) {
            DataShopOrderCart::mk()->where($map)->delete();
            UserAction::recount($this->unid);
            $this->success('移除成功！');
        }
        // 检查商品是否存在
        $gspec = DataShopGoodsItem::mk()->where(['ghash' => $data['ghash']])->findOrEmpty();
        $goods = DataShopGoods::mk()->where(['code' => $gspec->getAttr('gcode')])->findOrEmpty();
        if ($goods->isEmpty() || $gspec->isEmpty()) {
            $this->error('商品不存在！');
        }
        // 保存商品数据
        $data += ['gcode' => $gspec['gcode'], 'gspec' => $gspec['gspec']];
        if (($cart = DataShopOrderCart::mk()->where($map)->findOrEmpty())->save($data)) {
            UserAction::recount($this->unid);
