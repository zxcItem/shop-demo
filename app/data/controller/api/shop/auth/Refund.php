...
use app\data\model\shop\DataShopOrderRefund;
...
class Refund extends Auth
{
    ...
    public function get()
    {
        DataShopOrderRefund::mQuery(null, function (QueryHelper $query) {
    ...
    public function add()
    {
        ...
        $refund = DataShopOrderRefund::mk()->where($map)->findOrEmpty();
        if ($refund->isExists()) {
            $this->error('已存在售后单！');
        }
        // 上传图片转存
        ...
        if (($refund = DataShopOrderRefund::mk())->save($data)) {
    ...
    public function express()
    {
        ...
        $data['express_name'] = DataShopExpressCompany::mk()->where($map)->value('name');
        if (empty($data['express_name'])) {
            $this->error('无效快递公司！');
        }
        // 更新售后内容
        self::saveRefund(function (DataShopOrderRefund $refund) use ($data) {
