...
namespace app\data\controller\api\shop;
...
use app\data\model\shop\DataShopUserActionSearch;
...
class Goods extends Controller
{
    ...
    public function get()
    {
        $this->coupon = null;
        $this->cnames = null;
        DataShopGoods::mQuery(null, function (QueryHelper $query) {
            // 根据优惠券展示商品
            if ($couponCode = input('coupon')) {
                $where = ['code' => $couponCode, 'deleted' => 0];
                $userCoupon = DataShopUserCoupon::mk()->where($where)->findOrEmpty();
            ...
            if (!empty($vCates = input('cates'))) {
                $cates = array_filter(DataShopGoodsCate::items(), function ($v) use ($vCates) {
                    return $v['id'] == $vCates;
                });
                $this->cnames = null;
                if (count($cates) > 0) {
            ...
            if (!empty($code = input('code'))) {
                ...
                DataShopGoods::mk()->where(['code' => $code])->inc('num_read')->update([]);
            } else {
    ...
    {
        $this->success('获取分类成功', [
            'mark' => DataShopGoodsMark::items(),
            'cate' => DataShopGoodsCate::dtree(),
        ]);
    }
    ...
    {
        DataShopUserActionComment::mQuery(null, function (QueryHelper $query) {
    ...
    {
        $query = DataShopExpressCompany::mk()->where(['status' => 1, 'deleted' => 0]);
    ...
    {
        DataShopUserActionSearch::mQuery(null, function (QueryHelper $query) {
