...
class Admin extends Controller
{
    ...
    public function index()
    {
        ...
        DataWemallUserRelation::mQuery()->layTable(function () {
            $this->title = '会员用户管理';
            $this->upgrades = DataWemallConfigLevel::items();
    ...
    {
        DataWemallUserRelation::mQuery()->with('user')->mForm('form', 'unid');
    }
    ...
    public function parent()
    {
        if ($this->request->isGet()) {
            ...
            try {
                ...
                $parent = DataWemallUserRelation::mQuery()->where(['unid' => $data['puid']])->findOrEmpty();
                if ($parent->isEmpty()) {
                    $this->error('上级用户不存在！');
                }
                $relation = DataWemallUserRelation::withInit(intval($data['unid']));
