<?php

declare(strict_types=1);

namespace app\data\controller\wemall\user\coupon;

use app\data\model\wemall\DataWemallConfigCoupon;
use app\data\model\wemall\DataWemallConfigLevel;
use think\admin\Controller;
use think\admin\helper\QueryHelper;
use think\db\exception\DataNotFoundException;
use think\db\exception\DbException;
use think\db\exception\ModelNotFoundException;

/**
 * 抵扣卡券管理.
 * @class Config
 */
class Config extends Controller
{
    /**
     * 抵扣卡券管理.
     * @auth true
     * @menu true
     * @throws DataNotFoundException
     * @throws DbException
     * @throws ModelNotFoundException
     */
    public function index()
    {
        $this->type = $this->get['type'] ?? 'index';
        DataWemallConfigCoupon::mQuery()->layTable(function () {
            $this->title = '抵扣卡券管理';
            $this->types = DataWemallConfigCoupon::types;
        }, function (QueryHelper $query) {
            $query->like('name')->equal('status,type#mtype')->dateBetween('create_time');
            $query->where(['deleted' => 0, 'status' => intval($this->type === 'index')]);
        });
    }

    /**
     * 添加抵扣卡券.
     * @auth true
     */
    public function add()
    {
        $this->title = '添加抵扣卡券';
        DataWemallConfigCoupon::mForm('form');
    }

    /**
     * 编辑抵扣卡券.
     * @auth true
     */
    public function edit()
    {
        $this->title = '编辑抵扣卡券';
        DataWemallConfigCoupon::mForm('form');
    }

    /**
     * 修改抵扣卡券.
     * @auth true
     */
    public function state()
    {
        DataWemallConfigCoupon::mSave($this->_vali([
            'status.in:0,1' => '状态值范围异常！',
            'status.require' => '状态值不能为空！',
        ]));
    }

    /**
     * 删除抵扣卡券.
     * @auth true
     */
    public function remove()
    {
        DataWemallConfigCoupon::mDelete();
    }

    /**
     * 表单数据处理.
     */
    protected function _form_filter(array &$data)
    {
        if ($this->request->isGet()) {
            $this->types = DataWemallConfigCoupon::types;
            $this->levels = DataWemallConfigLevel::items();
            array_unshift($this->levels, ['name' => '全部', 'number' => '-']);
        } else {
            $data['levels'] = arr2str($data['levels'] ?? []);
        }
    }

    /**
     * 表单结果处理.
     */
    protected function _form_result(bool $result)
    {
        if ($result) {
            $this->success('卡券保存成功！', 'javascript:history.back()');
        } else {
            $this->error('卡券保存失败！');
        }
    }
}
