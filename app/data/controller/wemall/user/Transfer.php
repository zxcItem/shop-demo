...
class Transfer extends Controller
{
    ...
    {
        DataWemallUserTransfer::mQuery()->layTable(function () {
            $this->title = '代理提现管理';
    ...
    public function audit()
    {
        if ($this->request->isGet()) {
            DataWemallUserTransfer::mForm('audit', 'code');
        } else {
            $data = $this->_vali([
            ...
            $map = ['code' => $data['code']];
            $find = DataWemallUserTransfer::mk()->where($map)->find();
            if (empty($find)) {
                $this->error('不允许操作审核！');
            }
            ...
            if (DataWemallUserTransfer::mk()->strict(false)->where($map)->update($data) !== false) {
                $this->success('操作成功');
            } else {
                $this->error('操作失败！');
            }
