...
class Level extends Controller
{
    ...
    {
        DataWemallConfigLevel::mQuery()->layTable(function () {
            $this->title = '会员等级管理';
    ...
    public function add()
    {
        $this->max = DataWemallConfigLevel::maxNumber() + 1;
        DataWemallConfigLevel::mForm('form');
    }
    ...
    {
        $this->max = DataWemallConfigLevel::maxNumber();
        DataWemallConfigLevel::mForm('form');
    }
    ...
    public function _form_result(bool $state)
    {
        if ($state) {
            ...
            foreach (DataWemallConfigLevel::mk()->order($order)->select() as $number => $upgrade) {
    ...
    protected function _form_filter(array &$vo)
    {
        ...
        if ($this->request->isGet()) {
            $vo['number'] = $vo['number'] ?? DataWemallConfigLevel::maxNumber();
        } else {
            $vo['utime'] = time();
            if (empty($vo['number'])) {
