...
class Agent extends Controller
{
    ...
    {
        DataWemallConfigAgent::mQuery()->layTable(function () {
            $this->title = '代理等级管理';
    ...
    public function add()
    {
        $this->max = DataWemallConfigAgent::maxNumber() + 1;
        DataWemallConfigAgent::mForm('form');
    }
    ...
    {
        $this->max = DataWemallConfigAgent::maxNumber();
        DataWemallConfigAgent::mForm('form');
    }
    ...
    public function _form_result(bool $state)
    {
        if ($state) {
            ...
            foreach (DataWemallConfigAgent::mk()->order($order)->select() as $number => $upgrade) {
    ...
    protected function _form_filter(array &$vo)
    {
        if ($this->request->isGet()) {
            ...
            $vo['number'] = $vo['number'] ?? DataWemallConfigAgent::maxNumber();
        } else {
            $count = 0;
