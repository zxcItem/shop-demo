...
class Report extends Controller
{
    ...
    {
        $this->usersTotal = DataWemallUserRelation::mk()->cache(true, 60)->count();
        $this->goodsTotal = DataWemallGoods::mk()->cache(true, 60)->where(['deleted' => 0])->count();
        $this->orderTotal = DataWemallOrder::mk()->cache(true, 60)->whereRaw('status >= 4')->count();
        $this->amountTotal = DataWemallOrder::mk()->cache(true, 60)->whereRaw('status >= 4')->sum('amount_total');

        // 近十天的用户及交易趋势
        if (empty($this->days = $this->app->cache->get('Data.wemall.portals', []))) {
            $field = ['count(1)' => 'count', 'substr(create_time,1,10)' => 'mday'];
            // 统计用户数据
            $model = DataWemallUserRelation::mk()->field($field);
            $users = $model->whereTime('create_time', '-10 days')->group('mday')->select()->column(null, 'mday');
            // 统计订单数据
            $model = DataWemallOrder::mk()->field($field + ['sum(amount_total)' => 'amount']);
            $orders = $model->whereRaw('status>=4')->whereTime('create_time', '-10 days')->group('mday')->select()->column(null, 'mday');
            // 统计返佣数据
            $model = DataWemallUserRebate::mk()->field($field + ['sum(amount)' => 'amount']);
            $rebates = $model->whereTime('create_time', '-10 days')->group('mday')->select()->column(null, 'mday');
            // 统计余额数据
            $model = DataPaymentBalance::mk()->field($field + ['sum(case when amount>0 then amount else 0 end)' => 'amount1', 'sum(case when amount<0 then amount else 0 end)' => 'amount2']);
            ...
            for ($i = 15; $i >= 0; --$i) {
                ...
                $this->days[] = [
                    ...
                    '剩余余额' => DataPaymentBalance::mk()->whereRaw("create_time<='{$date} 23:59:59' and deleted=0")->sum('amount'),
                    '充值余额' => ($balances[$date] ?? [])['amount1'] ?? 0,
                    '消费余额' => ($balances[$date] ?? [])['amount2'] ?? 0,
                ];
            }
            $this->app->cache->set('Data.wemall.portals', $this->days, 60);
        }

        // 会员级别分布统计
        $levels = DataWemallConfigLevel::mk()->where(['status' => 1])->order('number asc')->column('number code,name,0 count', 'number');
        foreach (DataWemallUserRelation::mk()->field('count(1) count,level_code level')->group('level_code')->cursor() as $vo) {
