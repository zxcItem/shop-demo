...
use app\data\service\wemall\UserRefund;
...
class Order extends Controller
{
    ...
    public function index()
    {
        ...
        DataWemallOrder::mQuery()->layTable(function (QueryHelper $query) {
            $this->title = '订单数据管理';
            ...
            // 发货信息搜索
            $db = DataWemallOrderSender::mQuery()->dateBetween('express_time')
    ...
    public function audit()
    {
        if ($this->request->isGet()) {
            DataWemallOrder::mForm('', 'order_no');
        } else {
            $data = $this->_vali([
            ...
            $order = DataWemallOrder::mk()->where(['order_no' => $data['order_no']])->findOrEmpty();
            ...
            if (DataWemallOrder::mk()->strict(false)->where($map)->update($data) !== false) {
    ...
    public function cancel()
    {
        ...
        $order = DataWemallOrder::mk()->where($data)->findOrEmpty();
        if ($order->isEmpty()) {
            $this->error('订单查询异常！');
        }
        try {
