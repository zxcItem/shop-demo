...
use app\data\service\wemall\UserCoupon;
...
class Refund extends Controller
{
    ...
    public function index()
    {
        ...
        DataWemallOrderRefund::mQuery()->layTable(function (QueryHelper $query) {
            $this->title = '售后订单管理';
    ...
    protected function _form_filter(array &$data)
    {
        if ($this->request->isGet()) {
            ...
            try {
                $refund = DataWemallOrderRefund::mk()->findOrEmpty($data['id'] ?? 0);
                if ($refund->isEmpty()) {
                    $this->error('无效的售后单！');
                }
