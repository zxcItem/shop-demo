...
class Sender extends Controller
{
    ...
    public function index()
    {
        ...
        DataWemallOrderSender::mQuery()->layTable(function () {
            $this->title = '订单发货管理';
            $this->total = ['t0' => 0, 't1' => 0, 't2' => 0, 'ta' => 0];
            $this->address = sysdata('Data.wemall.address');
            // 订单状态统计
            $order = DataWemallOrder::mk()->whereIn('status', $this->oStatus)->where(['delivery_type' => 1]);
            $query = DataWemallOrderSender::mk()->whereRaw("order_no in {$order->field('order_no')->buildSql()}");
            ...
            // 订单搜索查询
            $db = DataWemallOrder::mk()->whereIn('status', $this->oStatus)->where(['delivery_type' => 1]);
    ...
    {
        DataWemallOrderSender::mForm('delivery_form', 'order_no');
    }
    ...
    protected function _delivery_form_filter(array &$vo)
    {
        if ($this->request->isGet()) {
            ...
            $delivery = DataWemallExpressTemplate::mk()->where($map)->findOrEmpty();
            if ($delivery->isEmpty() || empty($this->items = $delivery->getAttr('company'))) {
                $this->items = DataWemallExpressCompany::items();
            }
            ...
            $company = DataWemallExpressCompany::mk()->where($map)->findOrEmpty();
            if ($company->isEmpty()) {
